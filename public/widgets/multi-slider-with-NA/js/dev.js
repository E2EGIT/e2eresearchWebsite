...
$("document").ready(function () {
  ...
  var ArrStatementText = statementText.split("|");

  var outoutArry = [];
  var initArr = [];
  for (var j = 0; j < ArrStatementText.length; j++) {
    initArr[j] = j;
  }

  var suffVal = shuffleArray(ArrStatementText, initArr);

  for (var i = 0; i < ArrStatementText.length; i++) {
    if (i % 2 < 1) {
      oddEven = "odd";
    } else {
      oddEven = "even";
    }
    fnStatementtext = suffVal.pmtrArr[i].split("~");
    $("#binddata tbody").append(
      '<tr id="rowID_' +
        i +
        '"><td class="tbleft ' +
        oddEven +
        ...
        'px">' +
        fnStatementtext[0] +
        '</td><td class="tbmid"><div class="clearfix"><div class="fl">' +
        fnStatementtext[1] +
        '</div><div class="fr" style="text-align: right">' +
        fnStatementtext[2] +
        '</div></div><div id="slider-' +
        initArr[i] +
        '" class="slider1"></div></td><td><input type="button" class="na-bt" data-id="slider-' +
        initArr[i] +
        '" value="N/A"</td></tr>'
    );
    outoutArry[i] = "";
  ...
  var slider1 = $(".slider1")
    .slider({
      ...
      slide: function (event, ui) {
        ...
        outoutArry[currentId] = currentValue;
        console.log(currentValue, "currentValue");
        $("#slider-" + currentId).removeAttr("na");
        outputVal(outoutArry);
        $(this).removeClass("disabled");
        $(this).parent().removeClass("disabled_parent");

        $("[data-id=slider-" + currentId + "]").removeClass("disabled");
  ...
  $(".na-bt").click(function () {
    var $index = $(this).attr("data-id").split("-")[1];
    outoutArry[$index] = "99";
    outputVal(outoutArry);
    $(this).toggleClass("disabled");
    $("#slider-" + $index + " a span").text("0");
    $("#slider-" + $index).toggleClass("disabled");
    $("#slider-" + $index)
      .parent()
      .toggleClass("disabled_parent");
    $("#slider-" + $index).attr("na", "99");
    $("#slider-" + $index + " a").css("left", "0%");
  });
});
...
(function ($) {
  ...
  var extensionMethods = {
    pips: function (settings) {
      ...
      slider.element.addClass("ui-slider-pips").find(".ui-slider-pip").remove();
    ...
    float: function (settings) {
      ...
      slider.element.on(options.event, function (e, ui) {
        // Check if the click originated from an element with na='99'
        if ($(e.target).attr("na") === "99") {
          $(ui.handle).find(".ui-slider-tip").text(0); // Set tip to 0
